...
- **LPI завершение заявок**: всё хранится в едином `lpi_data.json` (поля `completedLocally`, `completedAt`, `taskId`), без отдельного файла; синхронизация через YouGile-задачи с `type:"lpi_data"` (или `"lpi_completed"` для старых задач) при загрузке
- **LPI синхронизация SQL↔YouGile**: при нажатии "Обновить" на вкладке "Таблица" — чтение SQLite через sql.js, обновление локальной БД, создание/обновление задач YouGile с полным JSON всех полей заявки (type: "lpi_data")
