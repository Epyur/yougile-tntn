...
### Текущая версия: 0.8.2 (в разработке)
- **Презентации**: полноэкранный режим прячет панель управления; переключение слайдов левой/правой кнопкой мыши; автопоказ по времени (кнопка ▶/⏸ Авто, интервал из шаблона/настроек); эффекты перехода (fade / fade+сдвиг / none); прогресс-бар; зацикливание. Настройки — в модалке «⚙ Показ» (⚙ → строка презентации), дефолты — из TemplateSpec (`slideIntervalSeconds`, `slideTransition`, `slideLoop`).
- **⚠️ Требует проверки**: 0.8.1 (реальная дата завершения `completeAt` в дашборде) — НЕ подтверждена, повторная проверка не менее чем через 12 часов после последнего синка: убедиться, что завершённые задачи получили `completeAt` в `yourbase/yougile_cache.json`, повторные синки не перезапрашивают одни и те же задачи, троттлинг `completeAtCheckedAt` (12 ч) работает.
- Если через 12+ часов в консоли снова появится сводка `YouGile backfill: still no completeAt for N task(s)` — значит, API не отдаёт дату для этих задач, и источник нужно уточнить.
