...
| 2 | **Задачи**: список, фильтры, дерево, чаты (вкладка group-chats + встроенный в детали задачи с файлами), inline-создание, ссылка «🌐 Открыть в YouGile» в деталях | ✅ | `ui/tasks-view.ts` |
...
- **Задачи: ссылка на задачу в YouGile** — в деталях задачи (вьюха «Задачи», `renderTaskDetailContent`, `src/ui/tasks-view.ts`) добавлена кнопка «🌐 Открыть в YouGile». Ссылка строится по рабочей схеме YouGile: `https://ru.yougile.com/team/{последний сегмент companyId}/#{idTaskProject}` (например `#TD-389`). Формат `https://yougile.com/task/{id}` не работает. `idTaskProject` приходит только в детальном ответе API (`GET /api-v2/tasks/{id}`) и в локальную БД не кэшируется; в hash-части фолбэк на `task.id`.
- **`607114d00703` в ссылке** — это последняя группа UUID `companyId` из настроек (`e6255265-7f76-4c8e-af38-607114d00703`); в веб-URL YouGile используется только хвост UUID рабочего пространства (`/team/{хвост}/#TD-389`).
