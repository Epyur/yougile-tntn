...
### Текущая версия: 0.8.1 (в разработке)
- Реализована учёт реальной даты завершения задач (`completeAt` из `GET /tasks/{id}`) в дашборде, дата-фильтрах и CSV-экспорте (модуль «Дашборд»).
- **⚠️ Функциональность НЕ подтверждена и требует повторной проверки не менее чем через 12 часов** после последнего синка: убедиться, что завершённые задачи получили `completeAt` в `yourbase/yougile_cache.json`, повторные синки больше НЕ перезапрашивают одни и те же задачи, а троттлинг `completeAtCheckedAt` (12 ч) работает.
