...
### Текущая версия: 0.8.4 (в разработке)
- **Дашборд (`completeAt`) — РАЗБЛОКИРОВАНО**: выяснено, что YouGile НЕ записывает `completedTimestamp` при завершении задачи через API (`PUT {completed:true}`), поле `completedTimestamp` в теле PUT отклоняется (HTTP 400), но **повторный plain `PUT {completed:true}` заставляет API записать дату**. Backfill для завершённых задач без даты теперь: `GET /tasks/{id}` → при отсутствии даты повторное завершение → снова `GET` → запись `completeAt`. Троттлинг `completeAtCheckedAt` (12 ч) сохранён.
- **⚠️ Требует проверки**: после синка убедиться, что все завершённые задачи получили `completeAt` в `yourbase/yougile_cache.json` и повторные синки больше не перезапрашивают одни и те же задачи. Для задач, завершённых через плагин до этого фикса, дата ≈ момент до-простановки (исходную дату API не хранил).
