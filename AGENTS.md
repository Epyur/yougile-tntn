...
- **LPI завершение**: только через LIMS (статус `received`) или YouGile-задачи (тип `lpi_completed`, `completed: true`); кнопка "Завершить заявку" из деталей удалена
