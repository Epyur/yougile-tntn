...
const CHANGELOG: Record<string, string[]> = {
  ...
  '0.3.3': [
    'Дашборд LPI: добавлен чекбокс "Серийная продукция" — если ЕКН указан (цифровой), то серийная; иначе опытный образец',
    'График "Топ продуктов": убраны подписи с числом испытаний на барах (оставлена только шкала)',
  ],
  '0.3.4': [
    'График "Топ продуктов": исправлено отображение названий (горизонтальный бар, yaxis.labels с maxWidth и word-break), контейнер растянут на 98% ширины',
    'Модальное окно выбора продуктов: теперь показывает только продукты, прошедшие фильтры дат и чекбоксы серийности/опытности',
    'Добавлен чекбокс "Опытная продукция" (серийная + опытная — независимые чекбоксы)',
    'Исправлен поиск в модальном окне продуктов (renderList замыкается на searchInput.value напрямую)',
  ],
};
